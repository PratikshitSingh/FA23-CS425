...
class Server:
        ...
        with self.rlock:
            ...
            for member_id, member_info in membershipList.items():
                ...
                if member_id in self.MembershipList:
                    ...
                    if (
                        member_info["incarnation"]
                        > self.MembershipList[member_id]["incarnation"]
                    ):
                    ...
                    # Update only if the received heartbeat is greater and both at the same incarnation
                        ...
                        member_info["heartbeat"] > current_heartbeat
                        and member_info["incarnation"]
                        == self.MembershipList[member_id]["incarnation"]
                    ):
                        self.MembershipList[member_id] = member_info
                        self.MembershipList[member_id]["time"] = time.time()
    ...
    def detectSuspectAndFailMember(self):
        ...
        with self.rlock:
            ...
            suspect_members_detected = [
                member_id
                for member_id, member_info in self.MembershipList.items()
                if member_info["time"] < failure_threshold_time
                and member_info["status"] != "Suspect"
            ]
            for member_id in suspect_members_detected:
                ...
                print(log_message)
            fail_members_detected = [
                member_id
                for member_id, member_info in self.MembershipList.items()
                if member_info["time"] < suspect_threshold_time
                and member_id not in self.failMemberList
                and member_info["status"] == "Suspect"
            ]
    ...
    def detectFailMember(self):
        ...
        with self.rlock:
            ...
            fail_members_detected = [
                member_id
                for member_id, member_info in self.MembershipList.items()
                if member_info["time"] < threshold_time
                and member_id not in self.failMemberList
            ]
            for member_id in fail_members_detected:
                self.failMemberList[member_id] = now
    ...
    def removeFailMember(self):
        ...
        with self.rlock:
            ...
            fail_members_to_remove = [
                member_id
                for member_id, fail_time in self.failMemberList.items()
                if fail_time < threshold_time
            ]
    ...
    def user_input(self):
        ...
        while True:
            user_input = input(
                "Enter 'join' to start sending, 'leave' to leave the group, 'enable suspicion' for GOSSIP+S mode and 'disable suspicion' for GOSSIP mode, 'list_mem' to list members and 'list_self' to list self info:\n"
            )
            if user_input == "join":
                self.enable_sending = True
                ...
                self.MembershipList = {
                    ...
                    for ip, port in [
                        (IP, DEFAULT_PORT_NUM) for IP in [self.ip, Introducor]
                    ]
                }
                ...
                try:
                    if self.enable_sending:  # Check if sending is enabled
                        ...
                        for peer in peers:
                            send_msg = self.json()
                            s.sendto(
                                json.dumps(send_msg).encode("utf-8"),
                                tuple(self.MembershipList[peer]["addr"]),
                            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--protocol-period",
        type=float,
        help="Protocol period T in seconds",
        default=0.25,
    )
    parser.add_argument(
        "-d", "--drop-rate", type=float, help="The message drop rate", default=0
    )
